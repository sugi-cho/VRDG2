Shader "mattatz/GPUParticleSystem/Update" {
	...
	Properties{
		...
		_SimulationTexSize ("Simulation Texture Size", Float) = 512
		_SimulationTexDeltaSize ("Simulation Texture Delta Size", Float) = 0.001953125
		_SimulationParticleCount ("Simulation Particle Count", Float) = 262144

		_Deceleration ("Deceleration", Range(0.0, 1.0)) = 0.95

		_LifetimeSpeed ("Lifetime Speed", Range(0, 1)) = 0.1
		_LifetimeRange ("Lifetime Range", Vector) = (0, 1, -1, -1)
	}
	...
	SubShader {
		...
		float _SimulationTexSize;
		float _SimulationTexDeltaSize;
		float _SimulationParticleCount;
		...
		float4 birth (float2 uv) {
			...
			float3 pos = random_point_on_sphere(seed) * _EmissionSize;
			// return float4(pos, nrand(seed));
		...
		float4 update_rotation(float2 uv) {
			...
			// float dt = _Config.z;
			float4 _SpinParams = float4(0.01, 0.02, 1, 1);
			float dt = 1.0;
		...
		f2o update(v2f IN) {
			...
			if (pos.a > 1.0) {
				float r = (IN.uv.x / _SimulationTexDeltaSize + (IN.uv.y / _SimulationTexDeltaSize) * _SimulationTexSize) / _SimulationParticleCount;
				if (r < _EmissionRate) {
					pos = birth(IN.uv);
					pos.a = 0.0;
					vel.xyz = 0.0;
