...
namespace mattatz {
    ...
    public class GPUComputeParticleSystem : MonoBehaviour {
        ...
        void Update() {
            ...
            */
            ...
            Dispatch(_UpdateDispatchKey);

            particleDisplayMat.SetVector("_Core", core.transform.position);
        ...
        void ApplyForce (Vector3 force) {
            updateShader.SetVector("_Force", force);
            Dispatch(_ApplyForceDispatchKey);
        }

        void Dispatch (string key) {
            var kernel = updateShader.FindKernel(key);
            if (kernel < 0) {
                Debug.LogWarning(key + " kernel doesnt exist.");
                return;
            }
        ...
        void OnRenderObject() {
            /*
            particleDisplayMat.SetBuffer("_Particles", buffer);
            particleDisplayMat.SetMatrix("_ModelMatrix", transform.localToWorldMatrix);
            particleDisplayMat.SetMatrix("_Object2World", transform.localToWorldMatrix);
            particleDisplayMat.SetMatrix("_InvModelMatrix", transform.worldToLocalMatrix);
            particleDisplayMat.SetPass(0);
